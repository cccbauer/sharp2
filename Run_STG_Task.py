...
murfi_FAKE = False
if murfi_FAKE == True:
    murfi_check = gui.Dlg(title='Warning:', labelButtonOK=' Turn OFF ', labelButtonCancel=' Keep ON ')
    murfi_check.addText('The MURFI simulator is ON.\n\nPress "Turn OFF" to turn off the MURFI simulator.')
    ok_data = murfi_check.show()  # show dialog and wait for Yes or No
    if murfi_check.OK:
        murfi_FAKE = False
...
expInfo = {'subject[xxx]':'', 'session[1/2/3/4]':'ses-nf', 'task[transferpre/transferpost]or[feedback]':'task-','run[xx]':'run-0','max':'1','min':'-1','mid':'0'}
dlg = gui.DlgFromDict(dictionary=expInfo, title=expName)
if dlg.OK == False: core.quit()  # user pressed cancel
expInfo['date'] = data.getDateStr()  # add a simple timestamp
expInfo['expName'] = expName

# SO sentence and question list as Pandas dataframe
SO_content = pd.read_csv(_thisDir + os.sep + 'SO_sentences.csv')
# setting word number as index column
SO_content['Index (corresponding to audio file name)'] = SO_content['Index (corresponding to audio file name)'].fillna(0.0).astype(str)
SO_content.set_index('Index (corresponding to audio file name)', inplace = True)
...
if exists >= 1:
    ...
    myDlg = gui.Dlg(title=str(exists) +' file(s) with this name already exist', labelButtonOK=' No ', labelButtonCancel=' Yes ')
...
#This section writes the BIDS events in tsv format
# --------- END OF OVERWRITING -------------------------------------
tsv_header = ['onset','duration','trial_type','block_count','block_type','response','response_time','stg_activity','smc_activity','feedback','sound_content','visual_content','number','stim_file','stim_duration','date']
...
# Setup the Window
win = visual.Window(size=(900, 900), fullscr=True, screen=1, allowGUI=False, allowStencil=False,
...
while continueRoutine:
    ...
    if trigger_button.status == STARTED:
        theseKeys = event.getKeys(keyList=['num_add', 't','5','a'])
...
baseline_save=[round(StartedTrialClock.getTime(),1),init_baseline,'baseline','n/a','baseline','n/a','n/a','n/a','n/a','n/a','n/a','+','n/a','n/a','n/a',expInfo['date']]
with open(tsvFile, "a",newline="") as f_tsv:
    tsv_writer = csv.writer(f_tsv, delimiter = '\t')
...
for thisBlock_order in block_order:
    ...
    for thisComponent in instructionComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
            #### Write to tsv ####
            instruction_save = [instruction_time,2,instruction_text,block_count+1,'instruction','n/a','n/a','n/a','n/a','n/a','n/a',instruction_text,'n/a','n/a','n/a',expInfo['date']]
            with open(tsvFile, "a",newline="") as f_tsv:
                tsv_writer = csv.writer(f_tsv, delimiter = '\t')
    ...
    for thisVoice_1 in voice_1:
        ...
        for thisComponent in transitionComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
                #### Write to tsv ####
                transition_save = [transition_time,0.5,'ISI',block_count+1,instruction_text,'n/a','n/a','n/a','n/a','n/a','n/a','+','n/a','n/a','n/a',expInfo['date']]
                with open(tsvFile, "a",newline="") as f_tsv:
                    tsv_writer = csv.writer(f_tsv, delimiter = '\t')
                    tsv_writer.writerow(transition_save) # write the header

        with contextlib.closing(wave.open(voice_file1)) as f:
        ...
        for thisComponent in stimulus_1Components:
            if hasattr(thisComponent, "setAutoDraw"):
                ...
                #print('voice 1:', voice_1.trialList[1])
                #print('voice 1:', voice_1)
                file1 = str(voice_file1).replace('\\','/')
                #r'C:/Users/rtsz_stim/Desktop/R33_rtsz/stimulus/audio/'+expInfo['subject[xxx]']+'/'+expInfo['subject[xxx]']+'_'
                #r'C:/Users/rtsz_stim/Desktop/R33_rtsz/stimulus/audio/R33_other_Female/JO_S'
                #print('file 1:',file1)
                if expInfo['subject[xxx]']+'/'+expInfo['subject[xxx]']+'_' in file1:
                    number1 = file1.replace(r'C:/Users/rtsz_stim/Desktop/R33_rtsz/stimulus/audio/'+expInfo['subject[xxx]']+'/'+expInfo['subject[xxx]']+'_','')
                    number1 = number1.replace('.wav', '')
                    content1 = SO_content.loc[number1]['Sentence']
                elif 'R33_Other_Female' in file1:
                    number1 = file1.replace(r'C:/Users/rtsz_stim/Desktop/R33_rtsz/stimulus/audio/R33_Other_Female/JO_S','')
                    number1 = number1.replace('.wav', '')
                    content1 = SO_content.loc[number1]['Sentence']
                elif 'R33_Other_Male' in file1:
                    number1 = file1.replace(r'C:/Users/rtsz_stim/Desktop/R33_rtsz/stimulus/audio/R33_Other_Male/JO_S','')
                    number1 = number1.replace('.wav', '')
                    content1 = SO_content.loc[number1]['Sentence']
                
                #### Write to tsv ####
                stimulus_1_save = [voice_file1_time,3.5,'voice_file',block_count+1,instruction_text,'n/a','n/a','n/a','n/a','n/a',content1,'+',number1,voice_file1,round(duration1,1),expInfo['date']]
                with open(tsvFile, "a",newline="") as f_tsv:
                    tsv_writer = csv.writer(f_tsv, delimiter = '\t')
    ...
    for thisVoice_2 in voice_2:
        ...
        with contextlib.closing(wave.open(voice_file2)) as f:
        ...
        #print('voice file 2:',voice_file2)
        # keep track of which components have finished
...
#        f = sf.SoundFile(voice_file2)
#        print('seconds = {}'.format(f.frames / f.samplerate))
#        print('file:', voice_file2)            
        ...
        for thisComponent in stimulus_2Components:
            if hasattr(thisComponent, "setAutoDraw"):
                ...
                #file2 = str(voice_file2).replace('\\','/')
                file2 = voice_file2.replace('\\','/')
                #print('file 2:',file2)
                if expInfo['subject[xxx]']+'/'+expInfo['subject[xxx]']+'_' in file2:
                    number2 = file2.replace(r'C:/Users/rtsz_stim/Desktop/R33_rtsz/stimulus/audio/'+expInfo['subject[xxx]']+'/'+expInfo['subject[xxx]']+'_','')
                    number2 = number2.replace('.wav', '')
                    content2 = SO_content.loc[number2]['Sentence']
                elif 'R33_Other_Female' in voice_file2:
                    number2 = file2.replace(r'C:/Users/rtsz_stim/Desktop/R33_rtsz/stimulus/audio/R33_Other_Female/JO_S','')
                    number2 = number2.replace('.wav', '')
                    content2 = SO_content.loc[number2]['Sentence']                  
                elif 'R33_Other_Male' in file2:
                    number2 = file2.replace(r'C:/Users/rtsz_stim/Desktop/R33_rtsz/stimulus/audio/R33_Other_Male/JO_S','')
                    number2 = number2.replace('.wav', '')
                    content2 = SO_content.loc[number2]['Sentence']

#                for path_type in (r'C:\Users\rtsz_stim\Desktop\R33_rtsz/stimulus/audio/'+expInfo['subject[xxx]']+'/'+expInfo['subject[xxx]']+'_',r'C:\Users\rtsz_stim\Desktop\R33_rtsz/stimulus/audio/R33_other_Female/JO_S',r'C:\Users\rtsz_stim\Desktop\R33_rtsz/stimulus/audio/R33_other_Male/JO_S'):
#                    number2 = voice_file2.replace(path_type, '')
#                number2 = number2.replace('.wav', '')
#                content2 = SO_content.loc[number2]['Sentence']

                #### Write to tsv ####
                stimulus_2_save = [voice_file2_time,3.5,'voice_file',block_count+1,instruction_text,'n/a','n/a','n/a','n/a','n/a',content2,'+',number2,voice_file2,round(duration2,1),expInfo['date']]
                with open(tsvFile, "a",newline="") as f_tsv:
                    tsv_writer = csv.writer(f_tsv, delimiter = '\t')
...
#        f = sf.SoundFile(voice_file3)
#        print('seconds = {}'.format(f.frames / f.samplerate))
#        print('file:', voice_file3)           

        with contextlib.closing(wave.open(voice_file3)) as f:
        ...
        for thisComponent in stimulus_3Components:
            if hasattr(thisComponent, "setAutoDraw"):
                ...
                file3 = str(voice_file3).replace('\\','/')
                #print('file 3:',file3)
                if expInfo['subject[xxx]']+'/'+expInfo['subject[xxx]']+'_' in file3:
                    number3 = file3.replace(r'C:/Users/rtsz_stim/Desktop/R33_rtsz/stimulus/audio/'+expInfo['subject[xxx]']+'/'+expInfo['subject[xxx]']+'_','')
                    number3 = number3.replace('.wav', '')
                    content3 = SO_content.loc[number3]['Sentence']
                elif 'R33_Other_Female' in file3:
                    number3 = file3.replace(r'C:/Users/rtsz_stim/Desktop/R33_rtsz/stimulus/audio/R33_Other_Female/JO_S','')
                    number3 = number3.replace('.wav', '')
                    content3 = SO_content.loc[number3]['Sentence']
                elif 'R33_Other_Male' in file3:
                    number3 = file3.replace(r'C:/Users/rtsz_stim/Desktop/R33_rtsz/stimulus/audio/R33_Other_Male/JO_S','')
                    number3 = number3.replace('.wav', '')
                    content3 = SO_content.loc[number3]['Sentence']

#                for path_type in (r'C:\Users\rtsz_stim\Desktop\R33_rtsz/stimulus/audio/'+expInfo['subject[xxx]']+'/'+expInfo['subject[xxx]']+'_',r'C:\Users\rtsz_stim\Desktop\R33_rtsz/stimulus/audio/R33_other_Female/JO_S',r'C:\Users\rtsz_stim\Desktop\R33_rtsz/stimulus/audio/R33_other_Male/JO_S'):
#                    number3 = voice_file3.replace(path_type, '')
#                number3 = number3.replace('.wav', '')
#                content3 = SO_content.loc[number3]['Sentence']
                #### Write to tsv ####
                stimulus_3_save = [voice_file3_time,3.5,'voice_file',block_count+1,instruction_text,'n/a','n/a','n/a','n/a','n/a',content3,'+',number3,voice_file3,round(duration3,1),expInfo['date']]
                with open(tsvFile, "a",newline="") as f_tsv:
                    tsv_writer = csv.writer(f_tsv, delimiter = '\t')
...
#        f = sf.SoundFile(voice_file4)
#        print('seconds = {}'.format(f.frames / f.samplerate))
#        print('file:', voice_file4)            

        with contextlib.closing(wave.open(voice_file4)) as f:
            frames4=f.getnframes()
            rate4=f.getframerate()
            duration4=frames4/float(rate4)
        ...
        for thisComponent in stimulus_4Components:
            if hasattr(thisComponent, "setAutoDraw"):
                ...
                file4 = str(voice_file4).replace('\\','/')
                #print('file 4:',file4)
                if expInfo['subject[xxx]']+'/'+expInfo['subject[xxx]']+'_' in file4:
                    number4 = file4.replace(r'C:/Users/rtsz_stim/Desktop/R33_rtsz/stimulus/audio/'+expInfo['subject[xxx]']+'/'+expInfo['subject[xxx]']+'_','')
                    number4 = number4.replace('.wav', '')
                    content4 = SO_content.loc[number4]['Sentence']
                elif 'R33_Other_Female' in file4:
                    number4 = file4.replace(r'C:/Users/rtsz_stim/Desktop/R33_rtsz/stimulus/audio/R33_Other_Female/JO_S','')
                    number4 = number4.replace('.wav', '')
                    content4 = SO_content.loc[number4]['Sentence']
                elif 'R33_Other_Male' in file4:
                    number4 = file4.replace(r'C:/Users/rtsz_stim/Desktop/R33_rtsz/stimulus/audio/R33_Other_Male/JO_S','')
                    number4 = number4.replace('.wav', '')
                    content4 = SO_content.loc[number4]['Sentence']

#                for path_type in (r'C:\Users\rtsz_stim\Desktop\R33_rtsz/stimulus/audio/'+expInfo['subject[xxx]']+'/'+expInfo['subject[xxx]']+'_',r'C:\Users\rtsz_stim\Desktop\R33_rtsz/stimulus/audio/R33_other_Female/JO_S',r'C:\Users\rtsz_stim\Desktop\R33_rtsz/stimulus/audio/R33_other_Male/JO_S'):
#                    number4 = voice_file4.replace(path_type, '')
#                number4 = number4.replace('.wav', '')
#                content4 = SO_content.loc[number4]['Sentence']

                #### Write to tsv ####
                stimulus_4_save = [voice_file4_time,3.5,'voice_file',block_count+1,instruction_text,'n/a','n/a','n/a','n/a','n/a',content4,'+',number4,voice_file4,round(duration4,1),expInfo['date']]
                with open(tsvFile, "a",newline="") as f_tsv:
                    tsv_writer = csv.writer(f_tsv, delimiter = '\t')
   ...
   # for thisTrial in trials:
    ...
    for thisComponent in ratingComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
            #### Write to tsv ####
    rating_save = [rating_time,8,'rating',block_count+1,instruction_text,lastRating[0],lastRating[1],'n/a','n/a','n/a','n/a',question,'n/a','n/a','n/a',expInfo['date']]
    with open(tsvFile, "a",newline="") as f_tsv:
        tsv_writer = csv.writer(f_tsv, delimiter = '\t')
...
# completed 1 repeats of 'block_order'
    ...
    if show_feedback==True:
        ...
        feedback_save=[feedback_time,8,'feedback',block_count+1,instruction_text,'n/a','n/a','n/a','n/a',round(fb_average,3),'n/a',feedbackColor,'n/a','n/a','n/a',expInfo['date']]
        with open(tsvFile, "a",newline="") as f_tsv:
            tsv_writer = csv.writer(f_tsv, delimiter = '\t')
            tsv_writer.writerow(feedback_save) # write the header
    else:
        nofeedback_save=[feedback_time,8,'nofeedback',block_count+1,instruction_text,'n/a','n/a','n/a','n/a','n/a','+','n/a','n/a','n/a','n/a',expInfo['date']]
        with open(tsvFile, "a",newline="") as f_tsv:
            tsv_writer = csv.writer(f_tsv, delimiter = '\t')
    ...
    for thisComponent in fixation_2Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    #### Write to tsv ####
    fixation_2_save=[fixation_2_time,fixation_duration,'fixation',block_count,instruction_text,'n/a','n/a','n/a','n/a','n/a','n/a','+','n/a','n/a','n/a',expInfo['date']]
    with open(tsvFile, "a",newline="") as f_tsv:
        tsv_writer = csv.writer(f_tsv, delimiter = '\t')
...
for i in range(0,len(all_vector)):
    ...
    if active_trial_count in range(26,37):
        activation_save = [ii,exp_tr,'activation',1,feedback_trial_type[0],'n/a','n/a',all_vector[i],all_vector_smc[i],'n/a','n/a','n/a','n/a','murfi','n/a',expInfo['date']]
        with open(tsvFile, "a",newline="") as f_tsv:
            tsv_writer = csv.writer(f_tsv, delimiter = '\t')
            tsv_writer.writerow(activation_save) # write 
    elif active_trial_count in range(62,75):
        activation_save = [ii,exp_tr,'activation',2,feedback_trial_type[1],'n/a','n/a',all_vector[i],all_vector_smc[i],'n/a','n/a','n/a','n/a','murfi','n/a',expInfo['date']]
        with open(tsvFile, "a",newline="") as f_tsv:
            tsv_writer = csv.writer(f_tsv, delimiter = '\t')
            tsv_writer.writerow(activation_save) # write 
    elif active_trial_count in range(100,111):
        activation_save = [ii,exp_tr,'activation',3,feedback_trial_type[2],'n/a','n/a',all_vector[i],all_vector_smc[i],'n/a','n/a','n/a','n/a','murfi','n/a',expInfo['date']]
        with open(tsvFile, "a",newline="") as f_tsv:
            tsv_writer = csv.writer(f_tsv, delimiter = '\t')
            tsv_writer.writerow(activation_save) # write 
    elif active_trial_count in range(136,147):
        activation_save = [ii,exp_tr,'activation',4,feedback_trial_type[3],'n/a','n/a',all_vector[i],all_vector_smc[i],'n/a','n/a','n/a','n/a','murfi','n/a',expInfo['date']]
        with open(tsvFile, "a",newline="") as f_tsv:
            tsv_writer = csv.writer(f_tsv, delimiter = '\t')
            tsv_writer.writerow(activation_save) # write 
    else:
        activation_save = [ii,exp_tr,'activation','n/a','intertrial'+str(intertrial_num),'n/a','n/a',all_vector[i],all_vector_smc[i],'n/a','n/a','n/a','n/a','murfi','n/a',expInfo['date']]
        with open(tsvFile, "a",newline="") as f_tsv:
            tsv_writer = csv.writer(f_tsv, delimiter = '\t')
