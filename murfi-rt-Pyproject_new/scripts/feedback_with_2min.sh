...
# Modified by Clemens Bauer November 2023
...
clear
    ...
    export MURFI_SUBJECTS_DIR="${absolute_path}/subjects/"
    export MURFI_SUBJECT_NAME=$subj
    singularity exec murfi2.1.sif murfi -f $subj_dir/xml/rest_2min.xml
...
clear
    ...
    export MURFI_SUBJECTS_DIR="${absolute_path}/subjects/"
    export MURFI_SUBJECT_NAME=$subj
    singularity exec murfi2.1.sif murfi -f $subj_dir/xml/experiencesampling.xml

fi

if  [ ${step} = 2min_extract_rs_networks ]
then
clear
    echo "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
    echo "+ compiling resting state run into analysis folder"

    expected_volumes=100
    runstring="2min resting state runs should have ${expected_volumes} volumes\n"
    for i in {0..10};
    do
    ...
    echo "Using run ${rest_runA_num} for 2min single-run ICA"

    # merge individual volumes to make 1 file for each resting state run
    rest_runA_filename=$subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_run-01_bold'.nii.gz
    ...
    echo "+ computing resting state networks this will take about 5 minutes"
    

     # realign volumes pre-FEAT
    mcflirt -in $rest_runA_filename -out $subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_run-01_bold_mcflirt.nii.gz'

    # get median volume of 1st run
    fslmaths $subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_run-01_bold_mcflirt.nii.gz' \
        -Tmedian $subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_run-01_bold_mcflirt_median.nii.gz'
 
    # skullstrip median of 1st run & check the generated mask
    bet $subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_run-01_bold_mcflirt_median.nii.gz' \
        $subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_run-01_bold_mcflirt_median_bet.nii.gz' \
        -R -f 0.4 -g 0 -m 

    slices $subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_run-01_bold_mcflirt_median.nii.gz' \
        $subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_run-01_bold_mcflirt_median_bet.nii.gz' \
        -o $subj_dir_absolute/qc/rest_skullstrip_check_run1.gif

    # mask run1 by the mask from skullstriped median of 1st run
    fslmaths $subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_run-01_bold_mcflirt.nii.gz' \
        -mas $subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_run-01_bold_mcflirt_median_bet_mask.nii.gz' \
        $subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_run-01_bold_mcflirt_masked.nii.gz'
    
    ica_run1_input=$subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_run-01_bold_mcflirt_masked.nii.gz'
    reference_vol_for_ica=$subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_run-01_bold_mcflirt_median_bet.nii.gz'

    # update FEAT template with paths and # of volumes of resting state run
    cp fsl_scripts/basic_ica_template_single_run.fsf $subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_'$run'_bold'.fsf
    OUTPUT_dir=$subj_dir_absolute/rest/rs_network_2min
    sed -i "s#DATA#$ica_run1_input#g" $subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_'$run'_bold'.fsf
    sed -i "s#OUTPUT#$OUTPUT_dir#g" $subj_dir/rest/$subj'_'$ses'_task-rest2min_'$run'_bold'.fsf
    sed -i "s#REFERENCE_VOL#$reference_vol_for_ica#g" $subj_dir/rest/$subj'_'$ses'_task-rest2min_'$run'_bold'.fsf 

    # update fsf to match number of rest volumes
    sed -i "s/set fmri(npts) 250/set fmri(npts) ${rest_runA_volumes}/g" $subj_dir_absolute/rest/$subj'_'$ses'_task-rest2min_'$run'_bold'.fsf
    feat $subj_dir/rest/$subj'_'$ses'_task-rest2min_'$run'_bold'.fsf   
...
if [ ${step} = 2min_process_roi_masks ]
then
    clear
        echo "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
        echo "+ Generating 2min DMN & CEN Masks "
        echo "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
        

    # Set up file paths needed for mask creation
    # first look for ICA feat directory based on multiple runs (.gica directory)
    ica_directory=$subj_dir/rest/rs_network_2min.gica/groupmelodic.ica/
    if [ -d $ica_directory ]
    then
        ica_version='multi_run'

    # if ICA feat dir for multi-run ICA isn't present, look for single-run version
    elif [ -d "${subj_dir}/rest/rs_network_2min.ica/filtered_func_data.ica/" ] 
    then
        ica_directory="${subj_dir}/rest/rs_network_2min.ica/" 
        ica_version='single_run_2min'
    else
    ...
    if [ $ica_version == 'single_run_2min' ]
    then
        # ICs in native space
