...
then
	...
	--separator=" " \
	--add-combo="Step" --combo-values "setup|---------BASELINE/FOLLOW-UPs---------|2vol|resting_state|extract_rs_networks|process_roi_masks|register|experience_sampling|cleanup|-----------------FEEDBACK-----------------|2vol|register|transfer|feedback|experience_sampling|cleanup|-----------------------------------------|backup_reg_mni_masks_to_2vol" \
	--cancel-label "Exit" --ok-label "Run Selected Step")
	ret=$?
	# parse zenity output using space as delimiter
	read -a input_array <<< $input_string
