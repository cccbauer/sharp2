...
template_smc='SMN_brainmaskero2.nii.gz'
...
clear
    ...
    export MURFI_SUBJECTS_DIR="${absolute_path}/subjects/"
    export MURFI_SUBJECT_NAME=$subj 
    singularity exec murfi2.1.sif murfi -f $subj_dir_absolute/xml/rtdmn_smc.xml
fi
...
then
    ...
    smc_uthresh=$ica_directory/smc_uthresh.nii.gz
    ...
    smc2example_func=${ica_directory}/reg/template_smc2example_func.nii.gz
    ...
    echo "aquiii"
    echo ${examplefunc_mask} ${infile} ${template2example_func}
    ...
    smc_thresh=$ica_directory/smc_thresh.nii.gz
    ...
    cp ${smc_thresh} ${subj_dir}/mask/smc_native_rest.nii.gz

    # Display masks with FSLEYES
    if [ $ica_version == 'single_run' ]
    then
        fsleyes $examplefunc ${mni_lps2xample_func} ${dmn_thresh} -cm blue ${cen_thresh} -cm red ${smc_thresh} -cm green
    else
        fsleyes $examplefunc ${mni_lps2xample_func} ${dmn_thresh} -cm blue ${cen_thresh} -cm red ${smc_thresh} -cm green
    fi
...
then
    ...
    smc_mni=SMC_brainmaskero2_lps.nii.gz
