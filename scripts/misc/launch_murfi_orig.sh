...
then
	...
	--separator=" " \
	--add-combo="Step" --combo-values "setup|2vol|2min_resting_state|extract_rs_networks|process_roi_masks|register|experience_sampling|resting_state|feedback|cleanup|backup_reg_mni_masks_to_2vol" \
	--cancel-label "Exit" --ok-label "Run Selected Step")
	ret=$?
	# parse zenity output using space as delimiter
	read -a input_array <<< $input_string
